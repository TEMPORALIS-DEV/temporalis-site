...
// “مرن” — إذا كان عندك event مختلف بنعدله لاحقًا بدون ما يطيح الموقع
const ABI = [
...
export async function getStrategyHistoryOnchain(strategyKey: string): Promise<HistoryRow[] | null> {
  try {
    ...
    // نجرب event 1 ثم 2
    const logs1 = await c.queryFilter(c.filters.SignalPublished(null, idBytes32), from, latest).catch(() => []);
    const logs2 = await c.queryFilter(c.filters.StrategyScored(null, idBytes32), from, latest).catch(() => []);
    ...
    const rows = logs
      ...
      .map((l: any) => {
        ...
        const score = toUnit(l.args?.score ?? 0n);
        const confidence = toUnit(l.args?.confidence ?? 0n);
